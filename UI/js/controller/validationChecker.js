...
const userGender = (formGender) => myGender.includes(formGender.value);
...
const validateDepartment = (dep, job) => {
  ...
  myDepartment = myCompany.find((department) => department[0] === dep.value);
  if (myDepartment) {
    const jobList = myDepartment[1];
    const myJob = jobList.find((role) => role === job.value);
